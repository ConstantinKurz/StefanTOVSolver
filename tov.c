...
void TOV_C_ParamCheck(CCTK_ARGUMENTS)
{
  ...
  neos=0; 
/* read eos for simplicityi, units are MeV/fm**3 and 1/fm**3 */
...
void TOV_C_Integrate_RHS(CCTK_ARGUMENTS)
{
  ...
  for (star=0; star < TOV_Num_TOVs; star++)
  {
    ...
    star_i = star * TOV_Num_Radial;    //CCTK_INT TOV_Num_Radial: "The number of radial points for the ODE integration;
    const CCTK_REAL rho_central=TOV_Rho_Central[star]; //TOV_Rho_Central[10] "The central density"
    printf("HALLO %f",rho_central);
    ...
    if (!TOV_Use_Old_Initial_Data) //TOV_Use_Old_Initial_Data "Take old initial data into account (spacetime)? Default is no.
      if(CCTK_EQUALS(metric_type, "static conformal")) //metric type defined in parfile;
          /*->? conformal state*/
      {
        *conformal_state=1;
    ...
    /* - utility routine
      - fills an real-array 'tov_press' of size 'tov_num_radial' with value '0.0' */
    ...
    TOV_press_1d[star_i] = getpress(rho_central);
    /* TOV_r_1d    [star_i] = LOCAL_TINY;
    TOV_rbar_1d [star_i] = LOCAL_TINY;*/

    /* build TOV_r_1d[] */
    for (i=star_i+1; i < star_i+TOV_Num_Radial; i++)
      TOV_r_1d[i] = TOV_r_1d[i-1] + TOV_dr[star]; //only tiv_dr[0] occurs in par file -->?
...
#define RKLOOP for (int rk=0; rk<NUMVARS; rk++)
                            ...
                            (TOV_Surface[star] < 0.0); i++)
    {
      ...
      TOV_C_Source_RHS(TOV_r_1d[i],
                     TOV_K, TOV_Gamma,
                     in_data, source_data);
      /* source_data ist leer wird durch TOV_C_Source_RHS
       * aber "hochgesetzt"
       * TOV_dr schrittweite
       * k1 also schrittweite mal ODEs
       * TOV_r und in_data werden schrittweise erhöht*/
      RKLOOP k1[rk] = TOV_dr[star] * source_data[rk];
      /* 2.Step*/
      RKLOOP in_data[rk] = old_data[rk] + 0.5 * k1[rk];
      /* die eigl k1,...,k4(also ableitungen) sind hier die TOV_C_Source_RHS*/
      ...
      local_rho = getrho(TOV_press_1d[i+1]);
      ...
      for (star=0; star<TOV_Num_TOVs; star++)
      {
        ...
        if (Perturb[star] == 0)
        {
          rho_point[star] = pow(press_point[star]/TOV_K,
                                1.0/TOV_Gamma);
          rho_point[star] = getrho(press_point[star]);
        }
        else
        {
          rho_point[star] = pow(press_point[star]/TOV_K,
        ...
        if (rho_point[star] > local_tiny)
        {
          eps_point[star] = press_point[star] / (TOV_Gamma - 1.0)
                                              /  rho_point[star];
          eps_point[star] = getmu(press_point[star])/rho_point[star]-1.0;
        }
        else
          eps_point[star] = 0.0;
